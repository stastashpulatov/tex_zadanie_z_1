...
1. **Инициализация графического интерфейса**: С помощью библиотеки React создается интерактивный веб-интерфейс с панелями управления, отображением статистики и журналом событий.

2. **Настройка правил обнаружения**: Пользователь может настроить параметры мониторинга:
   - Максимальный размер пакета (по умолчанию 1500 байт)
   - Включение/выключение проверки подозрительных портов
   - Активация защиты от DDoS-атак
   - Включение обнаружения сканирования портов

3. **Генерация и перехват сетевого трафика**: Программа симулирует реальный сетевой трафик, генерируя пакеты с различными протоколами (TCP, UDP, ICMP, HTTP, HTTPS) и параметрами.

4. **Анализ каждого пакета**: Для каждого перехваченного пакета выполняется проверка по установленным правилам:
   - Проверка размера пакета
   - Анализ используемых портов
   - Подсчет ICMP-пакетов для обнаружения flood-атак
   - Отслеживание обращений к различным портам для выявления сканирования

5. **Классификация трафика**: Пакеты классифицируются на три категории:
   - Нормальные (не вызывают подозрений)
   - Подозрительные (соответствуют одному или нескольким критериям угрозы)
   - Заблокированные (от заблокированных IP-адресов)

6. **Блокировка подозрительных IP**: При обнаружении угрозы пользователь может вручную заблокировать источник, после чего все пакеты с этого IP будут автоматически блокироваться.

7. **Логирование событий**: Все значимые события (обнаружение угроз, блокировка IP) записываются в журнал с указанием времени и типа события.

8. **Визуализация статистики**: В режиме реального времени отображается статистика:
   - Общее количество перехваченных пакетов
   - Количество нормальных пакетов
   - Количество подозрительных пакетов
   - Количество заблокированных пакетов

9. **Экспорт данных**: Возможность экспорта всех данных анализа в JSON-формате для дальнейшего исследования.
